...
FASER2DetectorConstruction::FASER2DetectorConstruction()
{
  ...
  if( opt == GeometricalParameters::magnetOption::SAMURAI ){
    ...
    fMagnetYokeThicknessY = 1*m; //GeometricalParameters::Get()->GetFASER2MagnetYokeThickY();
   
    fNTrackingStations = GeometricalParameters::Get()->GetNTrackingStations();
    fTrackingStationX = 3*m;//fMagnetWindowX + 0.5*m; //match magnet size + bending plane
    fTrackingStationY = 1*m;//fMagnetWindowY; // match magnet size
    ...
    G4ThreeVector DV_loc = preStationsCenter-G4ThreeVector(0,0, 10/2*m+5*mm)-T1;
    ...
    fFASER2Assembly->SetVisAttributes(DV_logVisAtt);
    ...
    for (int i= 0; i<fNTrackingStations; ++i) { 
      ...
      trkLayerLogical1->SetVisAttributes(stationVis);

      auto trkLayerPhysical1 = new G4PVPlacement(0, Tm, trkLayerLogical1, "TrkPhys", fFASER2Assembly, false, 0);
      ...
      trkLayerLogical2->SetVisAttributes(stationVis);

      auto trkLayerPhysical2 = new G4PVPlacement(0, Tp, trkLayerLogical2, "TrkPhys", fFASER2Assembly, false, 0);
  ...
  //   fNTrackingStations = GeometricalParameters::Get()->GetNTrackingStations();
  //   fTrackingStationX = 10*m; // 2*fMagnetInnerR + 0.5*m ; //match magnet size + bending plane (for now, FIXME?)
  //   fTrackingStationY = 10*m; // 2*fMagnetInnerR; // match magnet size
  //   fNScinBarsY = GeometricalParameters::Get()->GetNScintillatorBarsY();
  //   fNScinBarsX = GeometricalParameters::Get()->GetNScintillatorBarsX();
  //   fScinThickness = GeometricalParameters::Get()->GetScintillatorThickness();
  //   fTrackingStationGap = GeometricalParameters::Get()->GetTrackingStationGap();
